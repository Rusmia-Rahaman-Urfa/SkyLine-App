...
const Weather = () => {
    ...
    const fetchWeather = useCallback(async (searchCity) => {
        if (!searchCity || API_KEY === 'YOUR_API_KEY_HERE') {
            setModalMessage("Please enter a city name and ensure your OpenWeatherMap API key is set.");
            return;
        }
        ...
        try {
            ...
            if (data.cod === 200) {
                ...
                setModalMessage(`Could not find weather for "${searchCity}". Please check the spelling.`);
            }
        } catch (error) {
            console.error("Fetch error:", error);
            setModalMessage("An error occurred while fetching data. Check network or API configuration.");
        } finally {
            setLoading(false);
        }
    }, []);
    ...
    return (
        <div className="weather-container">
            <Title>SkyCast</Title> {/* Catchy name for the weather app */}
